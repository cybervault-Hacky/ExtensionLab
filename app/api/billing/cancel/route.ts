...
/** POST /api/billing/cancel — schedule cancellation at the end of the paid period. */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    requireSameOrigin(request);
    const user = requireApiUser(request);
    const limit = enforceRateLimit("billingChange", `${user.id}:${getClientIp(request)}`);
    if (!limit.ok) throw rateLimited(limit.retryAfterSeconds);
    const billing = await cancelSubscription(user, requestIdFrom(request));
