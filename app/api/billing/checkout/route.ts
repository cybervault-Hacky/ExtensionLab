 ...
 * price/amount/currency in the body is ignored.
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    requireSameOrigin(request);
    const user = requireApiUser(request);
    const limit = enforceRateLimit("billingCheckout", `${user.id}:${getClientIp(request)}`);
    ...
    return NextResponse.json({ url: result.url, reused: result.reused }, { status: 201, headers: { "cache-control": "no-store" } });
  } catch (error) {
