...
import { summarizeSession } from "@/lib/interactive/ai";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * "Summarize Session" (Phase 12): verified evidence counts plus a clearly
 * separated AI interpretation. The AI layer cannot promote an inference to a
 * verified fact; the envelope keeps the two apart.
 */
export async function POST(
  ...
  return withLogContext({ requestId }, async () => {
    try {
      ...
      const result = await summarizeSession(requestId, user.id, id);
