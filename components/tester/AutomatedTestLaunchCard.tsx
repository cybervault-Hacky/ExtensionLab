...
export function AutomatedTestLaunchCard({
  ...
  const launch = async () => {
    ...
    try {
      ...
      const created = (await response.json()) as { runId: string; sessionToken: string; suite: { total: number } };
      window.sessionStorage.setItem("extensionlab:test-token", created.sessionToken);
      ...
      if (!startResponse.ok) {
        ...
        window.sessionStorage.removeItem("extensionlab:test-token");
